...
function pMostrarPlantillasFiltradas() {
	...
	db.transaction (function (transaction) 
	{
		...
		var sql = "SELECT o.*, p.name as centro, o.reference,  v.name as proveedor FROM ordersTemplates as o, vendors as v , purchaseCenters as p WHERE o.idVendor=v.idVendor AND o.idPurchaseCenter=p.idPurchaseCenter AND o.idVendor="+provider+" AND o.idPurchaseCenter="+center+" ";
    ...
    function (transaction, result)
    {
      ...
      if (result.rows.length)
      {
        ...
        for (var i = 0; i < result.rows.length; i++) 
        {
	        ...
	        $("#pTxtNuevoPedidoPlantillaFecha").val(rowDb.documentDate);
...
function pMostrarTodasPlantillas() {
	...
	db.transaction (function (transaction) 
	{
		...
		var sql = " SELECT idInternalOrder as idTemplate, o.reference, o.idVendor, o.idPurchaseCenter,  o.documentDate, p.name as centro,  v.name as proveedor, s.icon, p.shortName, o.status as estado, observaciones as name, 1 as tieneError FROM ordersPending as o, vendors as v , purchaseCenters as p,  status as s, relPurchaseCenter_Vendors as r   WHERE r.idVendor=o.idVendor AND o.idPurchaseCenter=r.idPurchaseCenter "+filtroExtra+ " AND o.idVendor=v.idVendor AND o.idPurchaseCenter=p.idPurchaseCenter AND o.status=s.id AND o.tipoInterno="+TIPO_TEMPORAL_TEMPLATE+"  AND o.username='" + localStorage['usuario'] + "' AND o.unfinished=0 " +
              " UNION ALL " +		
						  "	SELECT idTemplate, o.reference, o.idVendor, o.idPurchaseCenter, o.documentDate, p.name as centro,  v.name as proveedor, s.icon, p.shortName, o.status as estado, o.name as name,0 as tieneError FROM ordersTemplates as o, vendors as v , purchaseCenters as p,  status as s , relPurchaseCenter_Vendors as r   WHERE r.idVendor=o.idVendor AND o.idPurchaseCenter=r.idPurchaseCenter "+filtroExtra+ " AND o.idVendor=v.idVendor AND o.idPurchaseCenter=p.idPurchaseCenter AND o.status=s.id";
		            ...
		            columns: [{
		               field: "cod_centr",
		                ...
		                headerTemplate: "<div style='position: relative; float: left'><a  data-role='button' role='button'> " 
		                                +"Estado"+ " <img id='sortPedidosPlan4' src='./images/sort_both.png' > </a></div>", 
