...
function formatearMoneda(n) {
	//************************************************************************
	//Funcion lista para adaptarla al nuevo web service de formato del precio
	//************************************************************************
	
	/*var formatPrecio="";
	var simbMil = localStorage['simboloMiles'];
	var simbDec = localStorage['simboloDecimales'];
	var simboloMod = localStorage['simboloMoneda'];
	var numDec = localStorage['numeroDecimales'];
	var posSimbolo = localStorage['posicionSimbolo'];
	
	var formateada = formatearMonedaIdioma(n,numDec,simbMil,simbDec);
	
	switch(simboloMod){
		case "$":
				if(posSimbolo=="AFTER"){   formatPrecio = '\u0024'+formateada;		}
				if(posSimbolo=="BEFORE"){  formatPrecio = formateada+'\u0024';		}
		case "€":
				if(posSimbolo=="AFTER"){   formatPrecio = '\u20AC'+formateada;		}
				if(posSimbolo=="BEFORE"){  formatPrecio = formateada+'\u20AC';		}
	}*/
...
function formatoFecha(columna){
	...
	/*var formatoFecha = localStorage['formatoFecha'];
	
	switch(formatoFecha){
		case "DD/MM/YYYY":
				if(mostrarHora==true){
					return "{0:"+formatoFecha+" HH:mm }" break;
				}else{
					return "{0:"+formatoFecha+"}"     break;
				}
		case "YYYY/MM/DD":
				if(mostrarHora==true){
					return "{0:"+formatoFecha+" HH:mm }"   break;
...
function kendoDateFormat() {
	...
	return DATE_FORMAT.replace("DD","dd").replace("YYYY","yyyy");
	
	
}

function formatearFechaHoraKendo(f /*,mostrarHora*/){
	
	return f;
	
	//NO FUNCIONA!!!!!!!!
	/*
	console.log("FORMATEANO FECHA = " + f + " ==> " +f.substring(8,10) + ", " +(parseInt(f.substring(5,7))-1)+ ", " +f.substring(0,4)+ ", " +f.substring(11, 13)+ ", " + f.substring(14, 16) );
	...
	*/
	...
	switch(formatoFecha){
		case "DD/MM/YYYY":
				if(mostrarHora==true){
					if (f != null){   return new Date(f.substring(8,10),(f.substring(5,7)-1),f.substring(0,4),f.substring(11, 13), f.substring(14, 16), 00, 00); break;  }
					else {            return new Date(0,0,0,0, 0, 00, 00);                                                                                      break;       }
				}else{
					if (f != null){ 	return new Date(f.substring(8,10),(f.substring(5,7)-1),f.substring(0,4));                                                break;    }
					else { 						return new Date(0,0,0);                                                                                                    break;      }
				}
		case "YYYY/MM/DD":
				if(mostrarHora==true){
					if (f != null){   return new Date(f.substring(0, 4),(f.substring(5,7)-1),f.substring(8, 10),f.substring(11, 13), f.substring(14, 16), 00, 00); break;  }
					else {            return new Date(0,0,0,0, 0, 00, 00);                                                                                          break;   }
				}else{
					if (f != null){   return new Date(f.substring(0, 4),(f.substring(5,7)-1),f.substring(8, 10));                                                    break;   }
					else {            return new Date(0,0,0);                                                                                                         break;    }
				}
	}
	*/
