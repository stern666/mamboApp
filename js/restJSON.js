...
function restPurchaseCenterJSON() {
		...
		$.ajax({
			  ...
			  contentType: 'application/json',
				...
				error: function (uri) { errorDeWS(uri); }
    ...
    db.transaction ( 
		  function (transaction) 
		  {
			    ...
			    for (i=0; i < querys.length; i++) {  	
			  	...
			  	localStorage['cargandoNumTotalItems'] = localStorage['cargandoNumTotalItems'] - 1;
...
//f_fin="2013-10-01";
	...
	$.ajax({
		...
		type:"GET",
		success:addOrders,
		error: function (uri) { errorDeWSOrders(uri);  }
				...
				querys[y]='INSERT OR IGNORE INTO settings (reference, name, value)  VALUES ("0","'+name+'", "'+value+'")';; 
					switch(name) {
						...
						case "CURRENCY":
							...
							break;
						case "THOUSANDS_MARK":
							THOUSANDS_MARK = value;
							break;
						case "DATE_FORMAT":
							DATE_FORMAT = value;
							break;
						case "CURRENCY_SIGN_PLACE":
							CURRENCY_SIGN_PLACE = value;
							break;
						case "EMAIL_ADDRESS":
							EMAIL_ADDRESS = value;
							break;
...
function errorDeWS(uri , error ) {
	    ...
	    console.log("ERROR DE WS 13/05");
	    console.log(uri);
	    console.log(error);

		insertLog(tipoLogWS,categoriaLogWSError,"Error al llamar al WS " + uri,"");
		
		//if (errorCargaInicial==0) {
		
			//alert("error"+uri);
			$("#cargaDialogTextAC").html("Error al cargar los datos. <br> Si desea recargar pulse aceptar. <br> Si desea continuar con los datos antiguos pulse cancelar.");
			$("#cargaDialogAC").popup("open");
			errorCargaInicial=1;
